    ...
    const handleDelete = async () => {
        try {
            const result = await fetch(
                `${import.meta.env.VITE_API_URL}/places/${place[0].id}`,
                {
                    method: "DELETE",
                }
            );
                ...
                <div className="elementPlaceContainer">
                    {place.map((element, index) => {
                        return (
                            <>
                                <div
                                    className="detailsPlaceLeftSection"
                                    key={index}
                                >
                                    <img
                                        className="detailsPlaceImage"
                                        src={element.image}
                                    />

                                    <div className="averageRate">
                                        Note générale: {avgRating}{" "}
                                        {renderStarRates()} ({ratingsCount})
                                    </div>
                                ...
                                <div className="elementDetailsPlaceContainer">
                                    <h1 className="detailsPlaceTilte">
                                        {element.name}
                                    </h1>
                                    <p className="detailsPlaceDescription">
                                        {element.description}
                                    </p>
                                    ...
                                    <div className="precisionDetailsPlace">
                                        <div className="mapsDetailsPlace">
                                            Future maps
                                        </div>
